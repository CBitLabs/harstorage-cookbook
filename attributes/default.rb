...
when 'debian'
    default['harstorage']['packages'] = %w[lib32stdc++6 python python-cairo python-rsvg python-setuptools python-pastescript]
when 'rhel', 'fedora'
...
default['harstorage']['supervisor']['command'] = "#{node['harstorage']['virtualenvhelper']} #{node['harstorage']['virtualenv-activate']}  #{node['harstorage']['virtualenv-python']} /usr/bin/paster serve #{node['harstorage']['ini']} --log-file #{node['harstorage']['logfile']}"
